...
/* UMA LISTA ENCADEADA É O MESMO QUE UMA HASH DE UMA UNICA POSIÇÃO (DICA DE ALUNOS); */
...
char **insere_palavrachave_hash(Hash hash, char *diretorio, int tamanho) { /* Retorna um vetor com as palavras chaves; */
	...
	for (i = 0; i < strlen(palavra); i++) {
        ...
        insere_palavra_hash(hash, palavra);
...
void insere_pagina_lista(Hash hash, char *diretorio, int tamanho) {
    ...
    while (fscanf(arquivo, "%c", &letra) != EOF) {
    	if (isalpha(letra)) { /* Verifica se é uma letra; */ 
            ...
            palavra[posicao] = '\0';
            posicao = 0;
            nodo = busca_palavra_hash(hash, palavra); /* Verifica se existe palavra-chave correspondente; */
            if (nodo != NULL){
		insere_pagina(hash, palavra, pagina); /* Se sim, insere o indice na lista da palavra; */
            }
	}
...
void escreve_palavra_pagina(Hash hash, char *diretorio_entrada, char *diretorio_saida, char **palavras, int tamanho){
    int i;
    for (i = 0; i < tamanho; i++) {
        escreve_arquivo(hash, palavras[i], diretorio_saida);
        free(palavras[i]);
    }
    free(palavras);	
    return;
